...
if (
    ...
    $stmt = $conn->prepare('UPDATE topicos SET capitulo_id = ?, descricao = ?, ordem = ? WHERE id = ?');
    $stmt->bind_param('isii', $capitulo_id, $descricao, $ordem, $id);
    if ($stmt->execute()) {
        header('Location: ../views/' . $redirect . '&sucesso=topico_editado');
        exit();
    } else {
